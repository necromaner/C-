...
TEST(change,set_To_String2) {
    ...
    y.erase(12345);
    y.erase(2);
    EXPECT_EQ("2,4-6",other.set_To_String(y, CUT2,CUT3)) << "2,4-6";
    y.erase(4);
    EXPECT_EQ("2,4,6",other.set_To_String(y, CUT2,CUT3)) << "2,4,6";
    y.insert(7);
    EXPECT_EQ("2,4,6,8",other.set_To_String(y, CUT2,CUT3)) << "2,4,6";
}
...
TEST(File,get_set) {
    ...
    if(file.setFile(12345, 100, 10)>=0){
        EXPECT_EQ(12345, file.getSize()) << 1;
        EXPECT_EQ(100, file.getBlock()) << 2;
        EXPECT_EQ(10, file.getSend()) << 3;
        EXPECT_EQ(124, file.getBlockMaxNum()) << 4;
        EXPECT_EQ(45, file.getBlockEndSize()) << 5;
        EXPECT_EQ(5, file.getBlockEndNum()) << 6;
        EXPECT_EQ(5, file.getBufEndSize()) << 7;
    }else{
        printf("文件信息错误");
    }
...
TEST(File,get_Block_Size) {
    ...
    error=file.setFile(100, 100, 6);
    if(error>=0) {
        EXPECT_EQ(100, file.get_Block_Size(0)) << "块大小等于文件大小";
    }
    error=file.setFile(100, 200, 6);
    if(error>=0) {
        EXPECT_EQ(100, file.get_Block_Size(0)) << "块大小大于文件大小";
    }
    error=file.setFile(100, 200, 100);
    if(error>=0) {
        EXPECT_EQ(100, file.get_Block_Size(0)) << "块大小大于文件大小";
    }
    error=file.setFile(100, 200, 150);
    if(error>=0) {
...
TEST(Udp,RE){
    ...
    for (int i = 0; i < RESIVE; ++i) {
        if(!re.isnum[i]){
            re.num[i]=*block_No.begin();
            re.isnum[i]=true;
            re.data[i]=udp.setSet_int(5-i);
            block_No.erase(block_No.begin());
        }
    }
    std::set<int>::iterator it;
    for(it=block_No.begin ();it!=block_No.end ();it++)
    {
        printf("%d ",*it);
    }
    printf("\n");
    for (int i = 0; i < RESIVE; ++i) {
        ...
        printf("%lld %d \n",re.num[i],re.isnum[i]);
        for(it=re.data[i].begin ();it!=re.data[i].end ();it++)
        {
            printf("%d ",*it);
        }
        printf("\n");
...
TEST(Udp,stringVector_SIZE){
    if(MAX_SEND==10){
        Udp udp;
        std::string s="1,2-5,6,8,10,15-300,4000,60000,62345,634585";
        std::vector<std::string>ss={"1,2-5,6,8","10,15-300","4000,60000","62345","634585"};
        std::vector<std::string>ss1=udp.stringVector_SIZE(s);
        std::vector<std::string>::iterator t ;
        for(t=ss1.begin(); t!=ss1.end(); t++)
            std::cout<<*t<<std::endl ;
        EXPECT_EQ(ss,ss1);
    }
}
TEST(Udp,vector_pushback){
    std::vector<int> s1;
    std::vector<int> s2;
    ...
    for(t=s1.begin(); t!=s1.end(); t++)
        std::cout<<*t<<" " ;
    std::cout<<std::endl;
    std::vector<std::string>::iterator t1 ;
    for(t1=s3.begin(); t1!=s3.end(); t1++)
        std::cout<<*t1<<" " ;
    std::cout<<std::endl;
}