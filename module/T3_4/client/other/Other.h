...
public:
    ...
    std::string set_To_String(std::set<int> y, const std::string &delim);//set序列化
    ...
    std::string set_To_String(std::set<int> y, const char &Cut1, const char &Cut2);

    std::set<int> string_to_Set(std::string y, const char &Cut2, const char &Cut3);

    std::vector<std::string> split(const std::string &str, const std::string &delim);

    void map_Insert(std::map<int, std::string> &maps, int num, std::string ss, const char &Cut2);

    std::map<int, std::set<int>>
    map_String_To_Set(std::map<int, std::string> &maps, const char &Cut2, const char &Cut3);
...
private:
    ...
    long long blockMaxNum;                            //块数量
    int blockEndSize;                                 //最后一个块长度
    int blockEndNum;                                  //最后一个块分割数量
    int bufEndSize;                                   //发送的最后一个长度

    void setBlockMaxNum();//块数量
    void setBlockEndSize();//最后一个块长度
    void setBlockEndNum();//最后一个块分割数量
    void setBufEndSize();
...
public:
    ...
    long long int getBlockMaxNum() const;//块数量
    int getBlockEndSize() const;//最后一个块长度
    int getBlockEndNum() const;//最后一个块分割数量
    int getBufEndSize() const;
    ...
    int getNowBufBegin(long long int blockNum, int bufNum);

    int getNowBufSize(long long int blockNum, int bufNum);

    int judgeBlockNum(long long int blockNum);

    int judgeBufNum(long long int blockNum, int bufNum);

    int getNowBlockNum(long long int blockNum);
};
